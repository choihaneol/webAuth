...
public class CertServiceImpl implements CertService {
	...
	@Override
	public int validateCertNew(Certificate_ cert, int result, CA certIssuer) throws CertificateException,
			InvalidKeyException, NoSuchAlgorithmException, NoSuchProviderException, SignatureException, CRLException {
		...
		while (currentCA.getCaType() != CAType.ROOTCA) { // self sign까지 올라가기
			...
			issuertmp = caRepository.findOneByCaId(issuertmp.getIssuerId());// issuer의 issuer 호출 
		...
		for (int i = 0; i < certChain.size(); i++) {
			...
			if (certChain.get(i).getCa().getCaType() != CAType.ROOTCA) { // ROOT제외
				...
				if (issuer.getCrlId() != 0) { // 발급자의 CRL존재 할 경우에만 CRL검증 거친다. 폐지리스트는(DB에서는) 대상인증기관이 아닌 발급기관의 CRL에 저장되어 있으므로
					...
					// if (cert.getCa().getCrlId() != 0 ) { // 발급자의 CRL존재 할 경우에만 CRL검증 거친다. 폐지리스트는(DB에서는) 대상인증기관이 아닌 발급기관의 CRL에 저장되어 있으므로.
	...
	public int validateCertNewNew(int result, Certificate_ cert, List<Certificate_> certChain, CA certIssuer)
		...
		for (int i = 0; i < certChain.size(); i++) {
			...
			if (certChain.get(i).getCa().getCaType() != CAType.ROOTCA) { // Root아닐시에만 CRL검증
				result = crlService.validateCRLNewNew(result, cert, certChain, certChain.get(i)); // 검증 certChain.get(i)기준
		...
		// (6)상위기관 폐지여부 확인
		if (certChain != null) { // 인증서 검증시에만 기능 수행, CRL검증시에는 수행X
