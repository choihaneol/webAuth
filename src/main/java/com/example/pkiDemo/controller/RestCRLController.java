...
public class RestCRLController {
	...
	//CRL 조회
		...
		public List<RevokedCertificate> getCRL(@PathVariable int caId) throws CertificateException, CRLException{
			...
			if (crl_ == null){ // CRL 없을시 목록으로 되돌아가기
		...
		public boolean validationCRL(@PathVariable int caId) throws InvalidKeyException, CertificateException,
		CRLException, NoSuchAlgorithmException, NoSuchProviderException, SignatureException  {

			boolean result = false; // 최종 검증 결과 result = 0 이면 "유효", result = 1이면, "폐지", 유효하지 않은 인증서로 시작  
			//int result boolean으로 수정 (https://wikidocs.net/220 )
