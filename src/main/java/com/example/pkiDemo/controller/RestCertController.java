...
public class RestCertController {
	...
	public Certificate register(@RequestBody Certificate_ request)
		...
		if (issuer.getCaType() == CAType.ROOTCA || issuer.getCaType() == CAType.ICA) {
			...
			cert = restCertService.generateCert(request.getUserName(), request.getCompany(), request.getEmail(),
					issuer.getCaType(), issuer);
	...
	public boolean delete(@PathVariable int certId) throws NoSuchAlgorithmException, NoSuchProviderException,
	...
	public Boolean validationCert(@PathVariable int certId) throws InvalidKeyException, CertificateException,
